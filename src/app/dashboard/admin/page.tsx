...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            const { data: reqs } = await supabase.from('service_requests').select('*').order('created_at', { ascending: false })
            setRequests(reqs || [])

            // Fetch designers for assignment
            const { data: des } = await supabase.from('designers').select('*').eq('verification_status', 'verified')
            setDesigners(des || [])
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] font-bold text-orange-600 uppercase mt-1">Status: Action Required</p>
                                    </div>
                                </div>
                            </div>
                        </Card>
