        ...
        <div className="space-y-12 animate-in fade-in duration-700 pb-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-2">
                    ...
                    <h1 className="text-5xl font-black text-white tracking-tighter uppercase italic">
                        Service <span className="text-blue-500">Factory</span>
                    </h1>
                    <p className="text-neutral-400 font-medium">Manage service provider capabilities and enlistments.</p>
                </div>
            </div>

            {success && (
                <div className="p-6 rounded-3xl flex items-center gap-4 font-bold border bg-emerald-500/10 border-emerald-500/20 text-emerald-400 animate-in zoom-in duration-300">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div className="space-y-6">
                    <Card className="p-8 border border-neutral-800 shadow-2xl rounded-[2.5rem] bg-neutral-900">
                        <h3 className="text-xl font-black text-white uppercase italic mb-6">Select Provider</h3>
                        <div className="space-y-4">
                            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wider block">Service Provider</label>
                            <select
                                value={selectedProviderId}
                                onChange={(e) => setSelectedProviderId(e.target.value)}
                                className="w-full h-14 px-4 rounded-2xl border-2 border-neutral-800 bg-neutral-950 text-white font-bold focus:outline-none focus:ring-4 focus:ring-blue-500/20 transition-all cursor-pointer hover:border-neutral-700"
                            >
                                ...
                                <div className="pt-6 animate-in slide-in-from-top-4 fade-in">
                                    <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl space-y-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <div className={`w-2 h-2 rounded-full ${updatedProvider(selectedProviderId)?.verification_status === 'verified' ? 'bg-emerald-500' : 'bg-orange-500'}`} />
                    ...
                    <Card className="p-8 border border-neutral-800 shadow-sm rounded-[2.5rem] bg-neutral-950 text-white">
                        ...
                        <ul className="space-y-3 text-xs font-medium text-neutral-400 leading-relaxed list-disc list-inside marker:text-blue-500">
                            ...
                            <li>These services will appear in the Concierge flow.</li>
                        </ul>
                    </Card>
                </div>

                {/* Services Grid */}
                <Card className="lg:col-span-2 border border-neutral-800 shadow-2xl rounded-[3rem] p-10 bg-neutral-900 relative overflow-hidden min-h-[600px]">
                    ...
                    {!selectedProviderId ? (
                        <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-40">
                            ...
                            <p className="text-xl font-black text-neutral-500 uppercase">Select a provider to begin</p>
                        </div>
                    ) : (
                        <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500 relative z-10">
                            <div className="flex justify-between items-center">
                                <h2 className="text-2xl font-black text-white uppercase">Service <span className="text-blue-500">Enlistment</span></h2>
                                <span className="bg-neutral-800 text-neutral-300 px-3 py-1 rounded-full text-xs font-bold border border-neutral-700">
                            ...
                            <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
                                    ...
                                    return (
                                        <div key={parent.id} className="space-y-4">
                                            <div className="flex items-center gap-2 pb-2 border-b border-neutral-800">
                                                <h3 className="font-extrabold text-lg text-white uppercase tracking-tight">{parent.name}</h3>
                                                <span className="text-[10px] font-bold text-neutral-400 bg-neutral-800 px-2 py-1 rounded-full">{subcategories.length} Sub-services</span>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                >
                                                    <Checkbox
                                                        ...
                                                        className="data-[state=checked]:bg-blue-600 data-[state=checked]:border-blue-600 w-5 h-5 border-neutral-600"
                                                    />
                                ...
                                {categories.every(c => !!c.parent_id) && categories.length > 0 && (
                                    <div className="text-center p-10 bg-orange-500/10 border border-orange-500/30 rounded-3xl text-orange-400 font-bold">
                                        Warning: No Root Categories found. Showing flat list.
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 text-left">
                                            {categories.map((cat) => (
                                                <label
                                                    key={cat.id}
                                                    className="flex items-center gap-4 p-4 rounded-2xl border-2 bg-neutral-950 border-orange-500/20 cursor-pointer"
                                                >
                                                    <Checkbox
                                                        checked={selectedServices.includes(cat.name)}
                                                        onCheckedChange={() => handleServiceToggle(cat.name)}
                                                        className="w-5 h-5 border-neutral-600 data-[state=checked]:bg-orange-500 data-[state=checked]:border-orange-500"
                                                    />
                                                    <span className="font-bold flex-1 text-white">{cat.name}</span>
                                                </label>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                {categories.length === 0 && (
                                    <div className="col-span-2 text-center py-10 text-neutral-500 font-medium">
                                        No categories found. Please add product categories first.
                                    </div>
                                )}
                            </div>

                            <div className="pt-8 border-t border-neutral-800 flex justify-end sticky bottom-0 bg-neutral-900/90 backdrop-blur-sm p-4 -mx-4 -mb-4 rounded-b-[3rem] z-20">
                                <Button
                                    onClick={handleSave}
                                    disabled={updating}
                                    className="h-14 px-8 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-blue-900/40 transition-all active:scale-95"
                                >
