...
export async function getApplicableMilestones(type: string, categoryId?: string) {
    ...
    // Find specific match
    const specific = (data as Record<string, unknown>[])?.find((t: Record<string, unknown>) => t.category_id === categoryId);
    if (specific) return specific.stages;

    // Find default for type
    const global = (data as Record<string, unknown>[])?.find((t: Record<string, unknown>) => !t.category_id);
